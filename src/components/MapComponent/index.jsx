...
const MapComponent = () => {
    ...
    const WMS_URL = process.env.REACT_APP_WMS_URL || `${PROXY}http://zs.zulugis.ru:6473/ws?`;
    const WMS_LAYER = process.env.REACT_APP_WMS_LAYER || 'world:world';
    const WMS_VERSION = process.env.REACT_APP_WMS_VERSION || '1.3.0';
    const USERNAME = process.env.REACT_APP_USERNAME || 'mo';
    const PASSWORD = process.env.REACT_APP_PASSWORD || 'mo';
        ...
        const wmsSource = new ImageWMS({
            ...
            imageLoadFunction: (image, src) => {
                fetch(src, {
                    ...
                    .then((response) => {
                        if (!response.ok) throw new Error(`Ошибка загрузки WMS: ${response.status}`);
                    ...
                    .catch((err) => {
                        console.error('Ошибка авторизованной загрузки WMS:', err);
                    });
            }
        });
